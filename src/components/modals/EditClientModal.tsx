// src/components/modals/EditClientModal.tsx - Contraste corrigido
'use client'
...
export default function EditClientModal({ isOpen, onClose, client, onClientUpdated }: EditClientModalProps) {
  ...
  const validateForm = () => {
    ...
    if ((formData.total_contract_value || 0) < 0) {
      newErrors.total_contract_value = 'Valor do contrato não pode ser negativo'
    }

    if ((formData.monthly_recurring_revenue || 0) < 0) {
    ...
    try {
      ...
      const { error: interactionError } = await supabase
        ...
        .insert([{
          ...
          description: `Dados do cliente ${updateData.company_name} foram atualizados`,
          outcome: 'Positivo',
          created_by: updateData.account_manager_id,
