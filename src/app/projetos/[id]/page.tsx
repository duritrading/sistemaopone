...
interface ProjectCommunication {
  ...
  participants: string[]; content?: string; sentiment?: string; 
  follow_up_actions?: string[]; creator?: { full_name: string };
}
...
const RiskMatrix = ({ risks }) => { 
  ...
  const riskMatrix = probabilityLevels.map(prob => impactLevels.map(imp => risks.filter(risk => risk.probability === prob && risk.impact === imp).length)); 
  return (
    <div className="grid grid-cols-5 gap-1">
      <div className="font-semibold text-sm text-center text-gray-600 pb-2"></div>
...
export default function ProjectDetailPage() {
  ...
  const handleDeleteDeliverable = async (deliverableId: string) => { 
    if (window.confirm('Tem certeza que deseja excluir este entregável?')) { 
      try { 
        ...
        alert('Falha ao excluir o entregável.'); 
      } 
    } 
  };
  ...
  const riskMetrics = { 
    ativos: risks.filter(r => r.status === 'Ativo').length, 
    mitigados: risks.filter(r => r.status === 'Mitigado').length, 
    ocorridos: risks.filter(r => r.status === 'Ocorrido').length 
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-4xl mx-auto bg-red-50 border border-red-200 rounded-lg p-8">
          ...
          <button onClick={() => router.push('/projetos')} className="mt-6 bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">
            Voltar para a Lista
          </button>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="border-b border-gray-200 mb-6">
            <nav className="flex space-x-8">
              <button 
                ...
                className={`py-4 px-1 border-b-2 font-medium ${activeTab === 'overview' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                Visão Geral
              </button>
              <button 
                onClick={() => setActiveTab('timeline')} 
                className={`py-4 px-1 border-b-2 font-medium ${activeTab === 'timeline' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                Cronograma
              </button>
              <button 
                onClick={() => setActiveTab('deliverables')} 
                className={`py-4 px-1 border-b-2 font-medium ${activeTab === 'deliverables' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                Entregáveis
              </button>
              <button 
                onClick={() => setActiveTab('risks')} 
                className={`py-4 px-1 border-b-2 font-medium ${activeTab === 'risks' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                Riscos
              </button>
              <button 
                onClick={() => setActiveTab('communication')} 
                className={`py-4 px-1 border-b-2 font-medium ${activeTab === 'communication' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                Comunicação
              </button>
            </nav>
                ...
                <InfoCard title="Cronograma" icon={Calendar}>
                  ...
                  <div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div className="bg-blue-600 h-2.5 rounded-full" style={{width: `${actualProgress}%`}}></div>
                    </div>
                  </div>
                </InfoCard>
                ...
                <InfoCard title="Tecnologias" icon={Code}>
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.filter(tech => tech.name).map(tech => (
                      <span key={tech.id} className="px-3 py-1 bg-gray-200 text-gray-800 text-sm font-medium rounded-full">
                        {tech.name}
                      </span>
                    ))}
                  </div>
          ...
          {activeTab === 'timeline' && ( 
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <TimelineKPI_Card title="Concluídos" value={timelineMetrics.concluidos} icon={CheckSquare} iconColor="text-green-500" />
                <TimelineKPI_Card title="Em Andamento" value={timelineMetrics.emAndamento} icon={Loader} iconColor="text-blue-500" />
          ...
          {activeTab === 'deliverables' && ( 
            <div className="space-y-6">
              ...
              <InfoCard title="Gerenciador de Entregáveis" icon={FileText} actions={
                <button onClick={handleAddNewDeliverable} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm font-semibold flex items-center gap-2 hover:bg-blue-700">
                  ...
                  {deliverables.length > 0 ? deliverables.map(item => ( 
                    <div key={item.id} className="p-4 border rounded-lg flex justify-between items-center hover:bg-gray-50/50 transition-colors">
                      ...
                      <div className="flex items-center gap-2">
                        <button className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-md" title="Download">
                          <Download className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleEditDeliverable(item)} className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-md" title="Editar">
                          <Edit className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDeleteDeliverable(item.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md" title="Excluir">
                    ...
                    <div key={risk.id} className="p-4 hover:bg-gray-50/50 flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-1">
                          <button className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-md" title="Editar">
                            <Edit className="w-4 h-4" />
                          </button>
                          <button className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md" title="Excluir">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                      </div>
          ...
          {activeTab === 'communication' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <TimelineKPI_Card title="E-mails" value={commsMetrics.emails} icon={Mail} iconColor="text-gray-500" />
              </div>
              
              <InfoCard title="Timeline de Comunicação" icon={MessageSquare} actions={
                <button 
                  onClick={handleAddNewCommunication} 
                  className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm font-semibold flex items-center gap-2 hover:bg-blue-700"
                >
                  <Plus className="w-4 h-4" />
                  Nova Comunicação
                </button>
                      ...
                      <div className="flex-1 pb-8">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className={`px-2 py-0.5 text-xs rounded-full ${
                                comm.sentiment === 'Positivo' ? 'bg-green-100 text-green-700' :
                                comm.sentiment === 'Negativo' ? 'bg-red-100 text-red-700' :
                                'bg-gray-100 text-gray-700'
                              }`}>
                                {comm.sentiment || 'Neutro'}
                              </span>
                            </div>
                            <p className="text-sm text-gray-500">
                              {formatDateTime(comm.created_at)} por {comm.creator?.full_name || 'Sistema'}
                            </p>
                          </div>
                        ...
                        <div className="mt-2 text-gray-700 bg-gray-50 p-3 rounded-md border">
                          ...
                          {comm.follow_up_actions && comm.follow_up_actions.length > 0 && (
                            <div className="mt-2">
                              <p className="text-xs font-medium text-gray-600 mb-1">Ações de Follow-up:</p>
