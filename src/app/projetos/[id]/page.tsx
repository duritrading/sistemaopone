      ...
      <div className="min-h-screen bg-gray-50">
        <div className="bg-white p-6 border-b border-gray-200 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-6"><KPI_Card title="Saúde" value={healthConfig?.text} icon={healthConfig?.icon} /><KPI_Card title="Progresso" value={`${actualProgress}%`} icon={BarChart3} />{isClient && <KPI_Card title="Dias Restantes" value={daysRemaining ?? '--'} icon={Clock} />}<KPI_Card title="Orçamento Usado" value={formatCurrency(project.used_budget)} icon={DollarSign} subtitle={`de ${formatCurrency(project.total_budget)}`} /></div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          {activeTab === 'communication' && ( <div className="space-y-6"><div className="grid grid-cols-1 md:grid-cols-4 gap-4"><TimelineKPI_Card title="Reuniões" value={commsMetrics.reunioes} icon={Users} iconColor="text-blue-500" /><TimelineKPI_Card title="Decisões" value={commsMetrics.decisoes} icon={ClipboardCheck} iconColor="text-green-500" /><TimelineKPI_Card title="Escalações" value={commsMetrics.escalacoes} icon={AlertCircle} iconColor="text-red-500" /></div><InfoCard title="Timeline de Comunicação" icon={MessageSquare}><div className="space-y-6">{communications.length > 0 ? communications.map(comm => ( <div key={comm.id} className="flex gap-4 relative"><div className="absolute left-5 top-10 bottom-0 w-px bg-gray-200"></div><div className="flex flex-col items-center z-10"><div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center">{getCommunicationIcon(comm.type)}</div></div><div className="flex-1 pb-8"><p className="font-medium text-gray-800">{comm.title}</p><p className="text-sm text-gray-500">{formatDate(comm.created_at)} por {comm.creator?.full_name || 'Sistema'}</p><div className="mt-2 text-gray-700 bg-gray-50 p-3 rounded-md border"><p className="text-sm">{comm.content}</p>{comm.participants && comm.participants.length > 0 && (<p className="text-xs text-gray-500 mt-2"><strong>Participantes:</strong> {comm.participants.join(', ')}</p>)}</div></div></div> )) : ( <p className="text-gray-500 py-8 text-center">Nenhuma comunicação registrada.</p> )}</div></InfoCard></div> )}
        </div>
      </div>
